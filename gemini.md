...
**NEW RULES 2/17/26** retain existing app structure - keeping backend \ front end separation, and within those, keeping one-time use scripts together, app files together, etc. comment all code. if a file is going to be modified added or removed that is NOT tracked on GitHub, instead of deleting code or files, comment out the code instead, keeping the files. If new code is added, be sure to add a code comment with each line or block of new code. Be sure all application actions - that is ingestion into database, etc, are logged to a .log file in program_logs
